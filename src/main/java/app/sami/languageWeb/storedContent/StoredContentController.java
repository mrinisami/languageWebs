...
public class StoredContentController {
    ...
    @GetMapping("/store-content/{fileName}/delete-uri")
    public StoredContentURIDto getDeleteUri(@RequestJwtSubject UUID subject,
                                            @PathVariable String fileName){
        return new StoredContentURIDto(storedContentService.getDeleteUri(subject, fileName));
    }
